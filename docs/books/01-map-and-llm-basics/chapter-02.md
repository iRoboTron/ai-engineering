...
# Имена лаб-специфичные и не совпадают со стандартными переменными других твоих проектов:
# export LAB_MEMORY_URL="http://100.69.146.20:443"
# export LAB_MEMORY_PROJECT="ai-labs"
...
# %% [markdown]
# # День 1 · structured outputs: вакансия → типизированный объект
#
# `response_format={"type": "json_schema", ...}` — режим, в котором провайдер ограничивает саму
# генерацию под переданную схему: модель физически не может вернуть поле не того типа или лишний
# ключ. Это надёжнее, чем «попроси вернуть JSON» в тексте промпта, потому что тот текстовый JSON
# всё равно приходится проверять руками — здесь показаны оба пути: строгий и запасной.
# %%
import json
import re
import sys
...
# %% [markdown]
# ## Схема — из кода, не из головы
#
# Схему для модели строит `Vacancy.model_json_schema()` — не нужно отдельно поддерживать текстовое
# описание формата ответа: оно всегда совпадает с реальным Python-классом, потому что берётся прямо
# из него.
# %%
...
# %% [markdown]
# ## Строгий режим, потом запасной
#
# `strict: True` в `json_schema` — провайдер обязан вернуть ответ ровно этой формы, лишние поля и
# несовпадение типов невозможны на уровне генерации. Не все провайдеры/модели через OpenRouter это
# поддерживают: при `BadRequestError` с упоминанием structured output пробуем запасной путь — та же
# схема текстом в промпте, а после ответа — обычная `model_validate_json`, которая одинаково ловит
# ошибку в обоих случаях.
# %%
...
# %% [markdown]
# ## Прогон по всем текстам вакансий
# %%
