...
«Как вы понимаете, что ваш RAG стал лучше после изменения?» — вопрос, на котором заканчивается большинство собеседований для кандидатов уровня junior: они отвечают «проверяю на нескольких вопросах». Ответ уровня middle: «golden-набор, метрики поиска и генерации, трейсы каждого запроса в проде, регрессионный гейт в CI, тренды по неделям». В днях 2 и 3 ты уже считал recall@k — сегодня добавляешь метрики ответа, судью, трейсинг и процесс. После этого дня у тебя будет то, чего нет у большинства: числа о качестве своей системы и место, где они хранятся.
...
К паре «вопрос → где ответ» из дня 2 добавляется **эталонный ответ** (reference) — короткий, написанный человеком, с фактами, которые обязаны быть в ответе. Без эталона можно измерить faithfulness и relevancy, с эталоном — correctness и context recall. Достаточно эталонов для 10–15 вопросов из 25: остальное — метрики без эталона.
...
3. **Temperature 0** и сильная модель — сильнее той, что генерирует ответ.
4. **Калибровка**: 5–10 примеров размечены человеком; согласие судьи с человеком измерено до того, как судье доверили тысячи оценок.
...
Что смотреть на дашборде: стоимость на tenant и на запрос, латентность по шагам и p95, доля ошибок по типам, доля «нет ответа», тренд оценок по дням. **PII**: промпты и ответы содержат персональные данные — маскирование телефонов и email до отправки в трейсы, срок хранения, доступ по ролям; для 152-ФЗ — self-hosted, что Langfuse даёт.
...
1. **На pull request** — детерминированный гейт за минуту: recall@5 на golden-наборе выше порога, схема ответа валидна, честный отказ на вопросах без ответа. Без судьи: дёшево и стабильно. В web-agent уже есть self-test round-trip перед переключением трафика — это его продолжение.
...
- **Пробелы**: нет трейсов по шагам, нет метрик генерации, нет судьи, нет датасета в системе, нет трендов дрейфа. Сегодня закрываются все, кроме последнего — он появится сам, когда трейсы копятся неделю.
