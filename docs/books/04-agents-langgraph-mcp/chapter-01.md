...
LangGraph встречается в вакансиях по агентам, но частоту проверяй своей выборкой вакансий, а не общим утверждением. Интервьюер спросит: чем агент отличается от цепочки, как вы защищаетесь от бесконечного цикла и перерасхода, как агент помнит контекст между сессиями, что такое MCP и зачем он, если есть function calling. Ты эксплуатируешь агента (Hermes) в проде, построил сервис памяти для агентов и ежедневно работаешь через MCP в Claude Code — но пока называешь это своими словами. Сегодня — индустриальными, и с кодом на LangGraph, без которого слово в резюме будет ложью.

## Агент — это цикл, а не магия

Вспомни пять шагов function calling из дня 1.
...
- **Ожидаемые ошибки — типизированным результатом модели**, неожиданные/авторизация/сбой учёта — остановкой и ERROR, не маскировкой под пустой поиск: «документ не найден, попробуй другой запрос» даёт модели шанс исправиться.
...
- **Checkpointer** сохраняет состояние по `thread_id`. Нужен один и тот же экземпляр хранилища: новый `MemorySaver()` забывает предыдущие вызовы, даже в одном процессе. Память годится для лабы; для рестарта процесса нужен durable SQLite/Postgres backend с корректным lifecycle. В async-графе используй async-вариант saver. Внешний эффект мог завершиться до checkpoint: для безопасного повтора записи нужен ключ идемпотентности.
- **`interrupt()`** приостанавливает узел и ждёт решения человека; при `Command(resume=...)` узел переисполняется с начала до interrupt, поэтому до него не должно быть неповторяемых эффектов. Это human-in-the-loop без костылей.
...
Готовый высокоуровневый агент — `create_agent` из LangChain 1.x — подходит для стандартного случая; собственный `StateGraph` нужен для своих проверок, ветвлений и лимитов. На собеседовании покажи, что умеешь и то и другое, и знаешь, когда что.
...
2. **Мягкий бюджет (soft limit)** — до вызова оцениваем стоимость входа, резервируем выход и задаём `max_tokens`; после вызова сверяем usage, проверяя лимит даже у финального ответа. Неизвестная цена/usage означает fail-closed, не «ноль». Это не жёсткая гарантия списаний: инструменты, конкурентность, ретраи и тарификация провайдера требуют отдельного учёта и внешней квоты ключа.
...
Всё это ты уже делаешь интуитивно с Hermes (systemd с рестартами, лимиты модели, смена модели по цене). Сегодня — явно и в коде, на синтетических данных; интеграции с приватными сервисами — отдельный opt-in.
...
**Function calling** — это формат разговора модели с твоим кодом внутри одного приложения.
