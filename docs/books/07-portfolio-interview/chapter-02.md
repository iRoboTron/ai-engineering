...
Запуск: открой `mock.ipynb` в VS Code и нажми ▶ Run All (60 вопросов по умолчанию — поменяй `N`, если нужно другое число), отвечай вслух, честно ставь оценки. Итог и список слабых вопросов — в `career/mock-1.md`. Ниже 70 % по какому-то дню — этот день в план недели 2 первым. Повтори mock через два дня.
